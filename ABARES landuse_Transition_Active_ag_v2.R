...
transitions_active_ag <- transitions_agind |>
  filter(
...
transitions_active_ag <- transitions_active_ag |>
  mutate(
...
# SEPARATE TOTALS FOR EACH TIME POINT
# ============================================================
from_totals_active_ag <- transitions_active_ag |>
  group_by(from_agind) |>
  summarise(total_Mha_2011 = sum(scaled_area_Mha), .groups = "drop")

to_totals_active_ag <- transitions_active_ag |>
  group_by(to_agind) |>
  summarise(total_Mha_2021 = sum(scaled_area_Mha), .groups = "drop")


# Check - these should show different values
from_totals_active_ag
to_totals_active_ag

#
# ============================================================
# COMBINED ACTIVE AG TOTALS CSV
# ============================================================
combined_totals_ag <- from_totals_active_ag |>
  rename(category = from_agind) |>
  left_join(
    to_totals_active_ag |> rename(category = to_agind),
    by = "category"
  )

write.csv(combined_totals_ag,
          "N:/Advances in Australian Farming Systems Paper/Section 1/landuse maps/transitions_summary/landuse_totals_ag_2011_2021.csv",
          row.names = FALSE)


# ============================================================
### STOP HERE AND CHECK
### doing this drops some area for example:
#The transition table counts pairs of (from, to) values. 
#When you filter transitions_active_ag, 
#you remove any row where either from_agind or to_agind is "Not agricultural industry"
# or "Grazing native vegetation".


# ============================================================
# Build separate label lookups
label_lookup_2011_active_ag <- setNames(
  paste0(from_totals_active_ag$from_agind, "\n", 
         round(from_totals_active_ag$total_Mha_2011, 1), " Mha"),
  from_totals_active_ag$from_agind
)

label_lookup_2021_active_ag <- setNames(
  paste0(to_totals_active_ag$to_agind, "\n", 
         round(to_totals_active_ag$total_Mha_2021, 1), " Mha"),
  to_totals_active_ag$to_agind
)
...
# ============================================================
from_labels <- transitions_active_ag |>
  group_by(from_agind) |>
  summarise(total = sum(scaled_area_Mha), .groups = "drop") |>
...
to_labels <- transitions_active_ag |>
  group_by(to_agind) |>
  summarise(total = sum(scaled_area_Mha), .groups = "drop") |>
  mutate(label = case_when(
...
p <- ggplot(transitions_active_ag,
